...
# with "OS" or "DIAG".
demo_type="MFG"
demo_volume_label="CLOUNIX-DIAG"
...
EOF
    ...
    demo_grub_entry="CLOUNIX $demo_type"
    cat <<EOF >> $grub_cfg
menuentry '$demo_grub_entry' {
        search --no-floppy --label --set=root $demo_volume_label
        echo    'Loading ONIE CLOUNIX $demo_type kernel ...'
        linux   /demo.vmlinuz $GRUB_CMDLINE_LINUX \$ONIE_EXTRA_CMDLINE_LINUX DEMO_TYPE=$demo_type
        echo    'Loading ONIE CLOUNIX $demo_type initial ramdisk ...'
        initrd  /demo.initrd
}
EOF
